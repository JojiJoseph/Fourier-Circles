...
parser.add_argument('-N', type=int, default=None, help='Number of circles to approximate the curve')
...
if N is None:
    N = len(target_curve)
last_queue: deque = deque(maxlen=int(N*0.75)) # Last queue to draw the reconstructed curve
input_to_fourier = np.interp(np.linspace(0, 1, N), np.linspace(0, 1, len(target_curve)), target_curve)
...
while True:
    img = np.zeros((512, 512, 3), np.uint8) # Create a black image
    cv2.putText(img, f'N = {N}', (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
    cv2.putText(img, f'Target', (10, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
    cv2.putText(img, f'Reconstructed', (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
    # Draw target
    for xy in target_curve:
        x = xy.real + 256
        y = xy.imag + 256
        cv2.circle(img, (int(x), int(y)), 2, (0, 255, 0), -1)
    for x, y in last_queue:
    ...
    for i in range(N):
        ...
        cv2.arrowedLine(img, np.int0((x, y)), np.int0((next_x, next_y)), (0, 255, 255), 1)
        x = next_x
        y = next_y
